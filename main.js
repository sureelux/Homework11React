...
function SumInfo(props)  {
  ...
  const stTitle = {
    color : props.color,
    fontSize : props.size==='big' ? '40px' : '40px'
  }
  return (
    <div className='suminfo'>
      {/* <h1 style={stTitle}>Sum = 0</h1> */}
      <h1 style={ { color: props.color, fontSize: '40px' } }>Sum = {sum}</h1>
    </div>
  )
}
...
function App() {
  ...
  <>
    ...
    <h1 className="text-center1">SNRU Sureelux Pangkhamhak</h1>
    <button className="btn-center" onClick={hdlAddCounter}>Add Counter</button>
    <SumInfo color="tomato" size="big" counters={counters}/> 
